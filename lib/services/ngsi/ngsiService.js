    ...
    return function(entityId, service, subservice, attributes, deviceInformation, callback) {
      ...
      */
        ...
        if (config.getConfig().authentication && config.getConfig().authentication.enabled) {
            ...
            if(deviceInformation && deviceInformation.trust && 
               (operationFunction.name == "sendRegistrations" || operationFunction.name == "createInitialEntity" )){
                logger.debug(context, 'TRUST ****', deviceInformation);
                
                var trust = deviceInformation.trust;
                delete deviceInformation.trust;
                operationFunction(entityId, attributes, deviceInformation, trust, callback);
                return;
            }
            ...
            groupService.find(service, subservice, entityType, function(error, deviceGroup) {
                ...
                if (deviceGroup) {
                    ...
                    var serviceConfig = config.getConfig().types[entityType].service || config.getConfig().service;
                    var subserviceConfig = config.getConfig().types[entityType].subservice || config.getConfig().subservice;

                    if(serviceConfig === service && subserviceConfig === subservice){
                        typeInformation = config.getConfig().types[entityType];    
                    } else {
                        logger.error(context, new errors.GroupNotFound(service, subservice).message);                                                       
                    }
                } else {
                ...
                if (typeInformation && typeInformation.trust) {
                    ...
                    async.waterfall([
                        function(callback) { 
                            security.getToken(typeInformation.trust, function(error, trust){
                                if(trust && deviceInformation)
                                    deviceInformation.trust  = trust;
                                callback(error, trust);
                            });
                        },
                    ...
                    operationFunction(entityId, attributes, deviceInformation, null, callback);
                }                
            });
        } else {
                operationFunction(entityId, attributes, deviceInformation, null, callback);
