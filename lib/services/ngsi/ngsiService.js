...
var request = require('request'),
    ...
    url = require('url'),
    context = {
        op: 'IoTAgentNGSI.NGSIService88 '
    },
    updateMiddleware = [],
    queryMiddleware = [];


function sslRequest(options){

    if(config.getConfig().ssl && config.getConfig().ssl.active == true){
        if(config.getConfig().ssl.keyFile)        
            options.key = fs.readFileSync(path.resolve("./../" + config.getConfig().ssl.keyFile));
        if(config.getConfig().ssl.certFile)                
            options.cert = fs.readFileSync(path.resolve("./../" + config.getConfig().ssl.certFile));          
        options.rejectUnauthorized = (config.getConfig().ssl.rejectUnauthorized) ? true : false;                
        if(config.getConfig().ssl.ca)
            options.ca =  fs.readFileSync(path.resolve("./../" + config.getConfig().ssl.ca));                               

        var address = url.parse(options.url)
        options.url = "https://" + address.host + address.pathname;
 ...
 */
function createRequestObject(url, deviceData, typeInformation) {
    var cbHost =  config.getConfig().contextBroker.host + ':' + config.getConfig().contextBroker.port,
        options,
        serviceContext = {},
        headers = {
    ...
    if (deviceData && deviceData.service) {
        headers['fiware-service'] = deviceData.service;
        serviceContext.service = deviceData.service;
    }

    if (deviceData && deviceData.service) {
        headers['fiware-servicepath'] = deviceData.subservice;
        serviceContext.subservice = deviceData.subservice;
    }

    if(typeInformation){
        if(config.getConfig().authentication && config.getConfig().authentication.enabled){
            headers['X-Auth-Token']  = typeInformation.token;
        }
    ...
    logger.debug(context, 'typeInformation Register', options.url);    
    logger.debug(context, 'typeInformation Register', options.headers);    
 ...
 */
function sendUpdateValue(entityName, attributes, deviceData, typeInformation, callback) {
    var options = createRequestObject('/v1/updateContext', deviceData, typeInformation),
        payload;
    
    if (deviceData && deviceData.staticAttributes) {
        attributes = attributes.concat(deviceData.staticAttributes);
    }
    ...
    async.waterfall([
        ...
        apply(applyMiddlewares, updateMiddleware, payload, deviceData)
        ...
        if (error) {
            ...
            request(sslRequest(options),
                generateNGSIOperationHandler('update', entityName, deviceData, typeInformation, options, callback));
        }
    });
}
 ...
 * @param {String} token            User token to identify against the PEP Proxies (optional).
 */
function sendQueryValue(entityName, attributes, deviceData, typeInformation, callback) {
    var options = createRequestObject('/v1/queryContext', deviceData, typeInformation);
    ...
    request(sslRequest(options),
        generateNGSIOperationHandler('query', entityName, deviceData, typeInformation, options,
            function(error, result) {
                if (error) {
                    callback(error);
                } else {
                    applyMiddlewares(queryMiddleware, result, deviceData, callback);
                }
            }));
}
...
function executeWithDeviceInformation(operationFunction) {
    function authenticate(deviceInformation, typeInformationObj, callback){
        logger.debug(context, 'executeWithDeviceInformation device info', deviceInformation);
        var functions = ['sendRegistrations', 'createInitialEntity'];
        var typeInformation = _.clone(typeInformationObj);
        if (config.getConfig().authentication && config.getConfig().authentication.enabled) {        
            
            if(deviceInformation && deviceInformation.token){
                typeInformation.token = deviceInformation.token;
                delete deviceInformation.token;
                logger.debug(context, 'DELETE TOKEN!!!', deviceInformation);
                callback(null, deviceInformation, typeInformation);
                return;
            } 
            
            if (typeInformation && typeInformation.trust) {
                logger.debug(context, 'typeInformation', typeInformation.trust);
                security.getToken(typeInformation.trust, function(error, token){
                    if(token && typeInformation && _.contains(functions, operationFunction.name)){                             
                        deviceInformation.token  = token;                               
                    }
                    typeInformation.token = token;                            
                    callback(error, deviceInformation, typeInformation);                    
                });
            } else
                callback(null, deviceInformation, typeInformation);                    
        } else         
    ...
    //this function check service and subservice attribute
    function checkServiceAttribute(typeInformationObj, callback){
        console.log(typeInformationObj);
        if(typeInformationObj && !typeInformationObj.service){     
            var error = new errors.MissingAttributes('Missing attributes Services and Subservice.');
            error.code = '400';
            logger.error(context, "Missing attributes: Services and Subservice");            
            callback(error);
        }else {
            callback(null, typeInformationObj);
        }
    }
    
    return function(entityId, service, subservice, attributes, deviceInformation, callback) {
        logger.debug(context, 'INICIO executeWithDeviceInformation entity', entityId);       
        
        var typeInformation;        
        var entityType = deviceInformation.type;  
                         
        async.waterfall([
            apply(deviceServices.findConfigurationGroup,deviceInformation),//return the configuration of group
            apply(checkServiceAttribute),
            apply(authenticate, deviceInformation),//return the typeinformation with token and functions sendRegistrations has token in deviceInformation
            apply(operationFunction, entityId, attributes)            
        ], callback); 
                
        
        logger.debug(context, 'GROUP REGISTERY DONE!!!');
        logger.debug(context, 'type iformation fora cond****', deviceInformation);
    };
}
...
exports.executeWithDeviceInformation =  intoTrans(context, executeWithDeviceInformation);
exports.createRequestObject = intoTrans(context, createRequestObject);
exports.sslRequest = intoTrans(context, sslRequest);


var deviceServices = require('./../devices/deviceService');