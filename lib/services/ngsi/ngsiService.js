...
function executeWithDeviceInformation(operationFunction) {
    return function(entityId, service, subservice, attributes, deviceInformation, callback) {
        logger.debug(context, 'executeWithDeviceInformation entity', entityId);
      /*
        logger.debug(context, 'executeWithDeviceInformation resource', service);
        logger.debug(context, 'executeWithDeviceInformation api', subservice);
        logger.debug(context, 'executeWithDeviceInformation atir', attributes);
      */
        
        var typeInformation;
        var entityType = deviceInformation.type;        
        groupService.find(service, subservice, entityType, function(error, deviceGroup) {
                if (deviceGroup) {
                    typeInformation = deviceGroup;
                } else if(config.getConfig().types[entityType]) {
                    typeInformation = config.getConfig().types[entityType];    
                } else {
        ...
        if (config.getConfig().authentication && config.getConfig().authentication.enabled  
            && _.contains(functions, operationFunction.name)) {
            //Avoid another token request in the same operation
            if(deviceInformation && deviceInformation.token){
                logger.debug(context, 'TRUST ****', deviceInformation);                
                typeInformation.token = deviceInformation.token;
                delete deviceInformation.token;
                operationFunction(entityId, attributes, deviceInformation, typeInformation, callback);
                return;
            }                        
        }
        ...
        if (config.getConfig().authentication && config.getConfig().authentication.enabled) {
            ...
            logger.debug(context, 'operationFunction.name', operationFunction.name);
                ...
                async.waterfall([
                    function(callback) {
                        security.getToken(typeInformation.trust, function(error, token){
                            ...
                            callback(error, typeInformation);
                        });
                    },
                    apply(operationFunction, entityId, attributes, deviceInformation)
                ], callback);
            } else {
                logger.error(context, new errors.SecurityInformationMissing(typeInformation.trust).message);  
                operationFunction(entityId, attributes, deviceInformation, typeInformation, callback);
            }                
        } else {
            //envia o cbHost do Type
            operationFunction(entityId, attributes, deviceInformation, typeInformation, callback);
        }    
    };
}
