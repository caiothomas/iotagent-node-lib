...
function generateUpdateActions(req, contextElement, callback) {
    ...
    function createActionsArray(attributes, commands, device, callback) {
        ...
        if (updateHandler) {
            console.log("updateHandler attributes", attributes)
            updateActions.push(
                async.apply(
                    updateHandler,
                    contextElement.id,
        ...
        if (commandHandler) {
            if (device.polling) {
                logger.debug(context, 'updateActions  pushCommandsToQueue DEVICE', device);                                    
                updateActions.push(
                    async.apply(
                        pushCommandsToQueue,
                        device,
                ...
                logger.debug(context, 'updateActions  commandHandler attributes', commands);                
                updateActions.push(
                    async.apply(
                        commandHandler,
                        contextElement.id,
