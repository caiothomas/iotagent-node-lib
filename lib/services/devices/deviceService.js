...
function createInitialEntity(entityType, deviceData, newDevice, token, callback) {
    
    logger.debug(context, 'deviceData', deviceData);
    logger.debug(context, 'newDevice', newDevice);

    
    var options = ngsiService.createRequestObject('/v1/updateContext', deviceData, token);    
...
function mergeDeviceWithConfiguration(fields, defaults, deviceData, configuration, callback) {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        if (deviceData && deviceData[fields[i]] && ['active', 'lazy', 'commands'].indexOf(fields[i]) >= 0) {
            logger.debug(context, 'eh device ',   confField );
        ...
        if (configuration && configuration[fields[i]] && ['attributes', 'lazy', 'commands'].indexOf(confField) >= 0) {
            logger.debug(context, 'eh configuration ',   confField );
...
function registerDevice(deviceObj, callback) {
    function checkDuplicates(deviceObj, innerCb) {
        logger.debug(context, 'CAIO THOMAS REGISTRO DE DEVICE');
    ...
    function prepareDeviceData(deviceObj, configuration, callback) {
        logger.debug(context, 'CAIO THOMAS 222');
    ...
    function completeRegistrations(error, deviceData) {
        ...
        async.waterfall([
            ...
            apply(ngsiService.executeWithDeviceInformation(createInitialEntity),deviceData.type, deviceData.service, deviceData.subservice, deviceData)
