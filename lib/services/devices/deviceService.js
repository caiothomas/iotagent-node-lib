 ...
 * @param {Object} newDevice        Device object that will be stored in the database.
 */
//(entityName, unregister, deviceData,token, callback) 
function createInitialEntity(entityType, deviceData, newDevice, typeInformation, callback) {
    
    var options = ngsiService.createRequestObject('/v1/updateContext', deviceData, typeInformation);    
    options.headers['fiware-correlator']= (domain.active && domain.active.corr) || uuid.v4();    
    options.json = {
                contextElements: [
                    {
    ...
    request(ngsiService.sslRequest(options), createInitialEntityHandler(deviceData, newDevice, callback));
}
...
function mergeArrays(original, newArray) {
       logger.debug(context, 'original ',    JSON.stringify(original, null, 5));
    ...
    logger.debug(context, 'merge differenceArray ',    JSON.stringify(differenceArray, null, 5));
    logger.debug(context, 'differenceNamesArray ',    JSON.stringify(differenceNamesArray, null, 5));
...
function mergeDeviceWithConfiguration(fields, defaults, deviceData, configuration, callback) {
    /*
    logger.debug(context, 'Fields ',    JSON.stringify(fields, null, 5));    
    logger.debug(context, 'CONFIGURATION ',    JSON.stringify(configuration, null, 5));    
    logger.debug(context, 'DEVICE DATA ',    JSON.stringify(deviceData, null, 5));
    logger.debug(context, 'Default ',    JSON.stringify(defaults, null, 5));
    */
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        if (configuration && configuration[fields[i]] && ['attributes', 'lazy', 'commands'].indexOf(confField) >= 0) {
            configuration[confField] = configuration[fields[i]].map(setDefaultAttributeIds);
        }                  
...
function findConfigurationGroup(deviceObj, callback) {
    function handlerGroupFind(error, group) {
        ...
        if(!effectiveGroup.service && !effectiveGroup.subservice){
            effectiveGroup.service = config.getConfig().service;
            effectiveGroup.subservice = config.getConfig().subservice;
        }
...
function registerDevice(deviceObj, callback) {
    ...
    function prepareDeviceData(deviceObj, configuration, callback) {
        ...
        logger.debug(context, 'deviceObj***** [%s]', deviceObj.polling);
        logger.debug(context, 'deviceData***** [%s]', deviceData.polling);
        ...
        if(!deviceData.service && !deviceData.subservice){
            if(!config.getConfig().service || !config.getConfig().subservice){
    ...
    function completeRegistrations(error, deviceData) {
        ...
        async.waterfall([ 
            apply(registrationUtils.sendRegistrations, deviceData.type, deviceData.service, deviceData.subservice, false, deviceData),
            apply(processContextRegistration, deviceData),
            apply(ngsiService.executeWithDeviceInformation(createInitialEntity),deviceData.id, deviceData.service, deviceData.subservice, deviceData)
        ], function(error, results) {
            if (error) {
                callback(error);
            } else {
                
              logger.debug(context, 'REGISTRO result** :\n%s', JSON.stringify(results, null, 4));
                logger.debug(context, 'types', config.getConfig().types[deviceData.type]);
                ...
                config.getRegistry().store(results, callback);
            }
        });
    }
...
function removeAllSubscriptions(device, callback) {
    function removeSubscription(subscription, callback) {
        subscriptions.unsubscribe(device.id, device.service,device.subservice, subscription.id, device, callback);
    }
        ...
        async.waterfall([
            ...
            if (error) {
                ...
                async.waterfall([
                    ...
                    processUnsubscribes,
                    apply(registrationUtils.sendRegistrations,  mergedDevice.type, mergedDevice.service, mergedDevice.subservice, true, mergedDevice),
...
function updateRegisterDevice(deviceObj, callback) {
    ...
    function getAttributeDifference(oldArray, newArray) {
        ...
        if (oldArray && newArray) {
            ...
            logger.debug(context, 'updateKeys', updateKeys);
    ...
    async.waterfall([
        ...
        apply(ngsiService.executeWithDeviceInformation(createInitialEntity), deviceObj.id, deviceObj.service, deviceObj.subservice),       
        apply(combineWithNewDevice, deviceObj),
        apply(registrationUtils.sendRegistrations, deviceObj.type, deviceObj.service, deviceObj.subservice, false),
