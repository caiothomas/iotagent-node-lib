 ...
 * @param {Object} deviceData       Object containing all the deviceData needed to send the registration.
 */
//deviceData.id, deviceData.type, null, unregister, deviceData
//function sendRegistrations(entityName, resource, apikey, attributes, deviceInformation, callback) {
//operationFunction, entityName, attributes, typeInformation
//entityName, attributes, typeInformation, token, callback
function sendRegistrations(entityType, unregister, deviceData, typeInformation, callback) {
        
    var options = ngsiService.createRequestObject('/NGSI9/registerContext', deviceData, typeInformation);

    if (!deviceData || !deviceData.type) {
        callback(new errors.TypeNotFound(null, entityType));
        return;
    }
    ...
    request(ngsiService.sslRequest(options), createRegistrationHandler(unregister, deviceData, callback));
}

exports.sendRegistrations = intoTrans(context, ngsiService.executeWithDeviceInformation)(sendRegistrations);
